...
class JsonReader {
   ...
   private:
    ...
    std::unordered_map<graph::VertexId, std::string_view>
    ...
    struct AddedEdgesHasher {
        size_t operator()(
            const std::pair<std::string_view, std::string_view> &p) const {
            return std::hash<std::string_view>()(p.first) ^
                   std::hash<std::string_view>()(p.second);
        }
    };
    std::unordered_set<std::pair<std::string_view, std::string_view>,
                       AddedEdgesHasher>
        added_adges_;
    ...
    bool IsEdgeAdded(const std::string_view from, const std::string_view to);
